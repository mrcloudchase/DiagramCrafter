...
def create_resources(rg_label, resources_list, resource_mapping):
    resources = {}
...
def generate_all_diagrams(config):
    mg_labels = config["mg_labels"]
    aad_label = config["aad_label"]
    mg_subs_dict = config["mg_subs_dict"]
    sub_labels = config["sub_labels"]
    sub_rg_dict = config["sub_rg_dict"]
    rg_resources_dict = config["rg_resources_dict"]
    resource_mapping = config["resource_mapping"]
    ...
    with Diagram("CAF Foundation Azure Landing Zone", show=False, filename="generated_diagrams/CAF_Foundation_Azure_Landing_Zone"):
    ...
    for rg_label in rg_resources_dict.keys():
        with Diagram(f"Resources in {rg_label}", show=False, filename=f"generated_diagrams/Resources_in_{rg_label}"):
            resources_list = rg_resources_dict[rg_label]
            create_resources(rg_label, resources_list, resource_mapping)
